...
| D-028 | From v0.1.3, enable the Tauri updater with a product-specific signing key and GitHub Releases endpoint for this repo only. | Header shows an Update action when a newer signed release is available. Do not reuse ME or TE Components updater keys or endpoints. Private key stays outside the repo (`%USERPROFILE%\.tauri\te-test-equipment-inventory-updater.key`). |
